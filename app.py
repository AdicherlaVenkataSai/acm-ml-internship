...
def get_rec(title):
    ...
    if title not in data['TRACK NAME'].unique():
        return ("OOPS! we haven't kept track of these title. \n Please do check if the namely is correctly entered")
    else:
        idx = indices[title]
...
def recommend():
    ...
    if type(r) == type('string'):
        return render_template('results.html', title = title, r = r, t = 's')
    else:
        return render_template('results.html', title = title , r = r, t = 'l')
